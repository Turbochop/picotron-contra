...
function camera_respawn_lerp(pl,current,target)
    if pl.respawn and pl.respawn < 15 then
        local amount = mid(0,pl.respawn/15,1)
...
-- If a trailing player is stuck behind a wall, dont scroll
...
if not halt and desired_cam_x > cam_x then
    cam_x = camera_respawn_lerp(lead,cam_x,desired_cam_x)
end
...
if not halt and p.x > cam_x + solo_front_x then
    local desired_cam_x = p.x-solo_front_x
    cam_x = camera_respawn_lerp(p,cam_x,desired_cam_x)
end
