...
function add_boss_bubble(_x,_y,_z)
    ...
    local distance=sqrt(closest)
    local dx,dy=0,0

    if distance>0 then
        dx=aim_x/distance*speed
        dy=aim_y/distance*speed
    end
...
--  p.flp1=self.x<.x and true or false
 p.flp0=(self.x+8)<(p.x+4) and true or false
 p.health-=1
 end
 end
        end,
        ...
        draw=function(self)
            ...
            sspr(
                enemysheet,
                self.frame[self.sprite],96,16,16,
                self.x,self.y,16,16,
